...
class AudioExtractorImplTest{
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun test_changeAudioIsFavoriteStatus()= runTest {
        audioExtractor.changeAudioIsFavoriteStatus(1L,true)
        with(audioExtractor.getAllAudio()){
            val updatedItem = find { it.id == 1L }
            assert(updatedItem != null)
            assert(updatedItem?.isFavorite == true)
        }
    }
}